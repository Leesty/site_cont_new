...
def leads_stats_placeholder(request: HttpRequest) -> HttpResponse:
    ...
    today_count = (
        Lead.objects.filter(user=user, status=Lead.Status.APPROVED, created_at__gte=today_start, created_at__lt=today_end).count()
    )
    yesterday_count = (
        Lead.objects.filter(user=user, status=Lead.Status.APPROVED, created_at__gte=yesterday_start, created_at__lt=yesterday_end).count()
    )
