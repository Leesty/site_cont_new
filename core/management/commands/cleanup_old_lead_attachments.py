"""
Удаление файлов скриншотов лидов старше 20 дней для экономии места.

Запуск:
  python manage.py cleanup_old_lead_attachments
  python manage.py cleanup_old_lead_attachments --days 20 --dry-run  # только показать
...
class Command(BaseCommand):
    help = "Удаляет вложения (скриншоты) лидов старше 20 дней."
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=20,
            help="Удалять вложения лидов старше N дней (по умолчанию 20).",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
